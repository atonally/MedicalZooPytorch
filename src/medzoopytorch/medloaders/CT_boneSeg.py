...
import medzoopytorch.augment3D as augment3D
import medzoopytorch.utils as utils
from medzoopytorch.medloaders.medical_loader_utils import create_sub_volumes

class CT_boneSeg(Dataset):
    """ This function is very similar to CT_organSeg but does not perform random 
        flips in the images during augmentation, instead performing random rotations"""
    ...
    def __init__(self, args, mode, samples, sample_idx, dataset_path, crop_dim=(32, 32, 32), load=False):
        ...
        if self.mode == 'train':
            ...
            subvol = '_vol_' + str(crop_dim[0]) + 'x' + str(crop_dim[1]) + 'x' + str(crop_dim[2])+ '_' + str(self.voxel_resamp[0]) + 'mm'
            self.sub_vol_path = args.outputfolder  + '/Data/bones_' + mode + subvol + '/'
            ...
            self.list = create_sub_volumes(list_IDsCTs, labels, dataset_name="CT_boneSeg",
                                           organs = args.organs,
